...
<div>
  ...
  <div class="container-fluid content-list">
    ...
    <div class="row list-body" v-for="list in requestLists" :class="requestLists.indexOf(list) % 2 === 0 ? 'row list-body' : 'row list-body gray'">
      <div class="col-md-2 list-row" :title="list.recruitCitys"><span class="badge badge-primary">{{ list.recruitCitys }}</span></div>
      <div class="col-md-3 list-row" :title="list.companyName"><span class="gap-right badge badge-success" v-if="list.companyTags && '' !== list.companyTags">{{list.companyTags}}</span>{{ list.companyName }}</div>
